...
- **OpenAI Compatible** - any service that implements the OpenAI speech API, configured with a custom endpoint URL, model, voice list and API key. Works with hosted providers such as [Gandr](https://gandr.ai) (paid, with a free tier) or a local server.
