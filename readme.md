...
Brought to you by Cohee, RossAscends and the SillyTavern community, SillyTavern is a user interface that is installable on your computer (or Android phone) that allows you to interact with text generation AIs (LLMs) to chat and roleplay with custom characters.

While SillyTavern originated as a modification of TavernAI 1.2.8 in February 2023.

Since then it has since grown with more active development than the original TavernAI, and has added many major features such as: 

* Mobile-friendly interface
* Multi-API connectivity (KoboldAI/CPP, Horde, NovelAI, Ooba, OpenAI+proxies, Poe, WindowAI(Claude!)), 
* Visual Novel-like Waifu Mode
* Horde Stable Diffusion generation
* TTS support (ElevenLabs, Silero, and built-in OS)
* WorldInfo (lorebooks)
* Customizable UI colors and window placement
* Auto-translate
* more prompt options than you'd ever want or need. 

There is also an optional Extras server which provides greater functionality:

* stronger and more diverse Stable Diffusion and TTS integrations
* Smart Context to enhance AI model long-term memory (ChromaDB)
* Autogenerated chat Summary.
...
SillyTavern itself is only a user interface, and has tiny hardware requirements. 

It will run on virtually any device capable of running NodeJS v18. 
